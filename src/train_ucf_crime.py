...
def mil_ranking_loss(y_pred, y_true):
    ...
    # We enforce that positive max score > negative max score + 1
    margin = 1.0
    ...
    sparsity_loss = torch.sum(pos_preds) * 0.00008
    
    # Temporal smoothness: neighboring scenes should have comparable scores
    smoothness_loss = torch.sum((pos_preds[:, 1:, :] - pos_preds[:, :-1, :]) ** 2) * 0.00008
